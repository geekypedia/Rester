...
?>
<head>
  <title>Lua Administration</title>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
   ...
   //     			configForm.action = configForm.action.replace("configure/","");
	...
	<link rel="shortcut icon" href="../../api/configure/favicon.ico" />    
  ...
  100% { transform: rotate(360deg); }
	...
	<script type="text/javascript">
		...
		function act(command, cb){
			...
			if(command == 'luarocks') {
				payload[command] = $('#luarocks').val();
				payload['prefix'] = $('#prefix').val();
			}
			if(command == 'luarocksstart') {
				payload['luarocks'] = 'start';
				payload['prefix'] = $('#prefix').val();
			}
...
<div class="panel panel-primary main-container">
  <div class="panel-heading center-text">Lua Administration</div>
  <div class="panel-body">
		...
		<form id='configForm' action="generate-config.php" method="post">
		    ...
		    <div class="form-group col-md-3">
		      <label for="version">LuaDist Version:</label>
		      <input type="text" class="form-control" id="version" placeholder="Enter LuaDist version" name="version" required value="0.9.8">
		    </div>
		    <div class="form-group col-md-6">
		      <label for="prefix">Project Path:</label>
		      <input type="text" class="form-control" id="prefix" placeholder="Enter project path" name="prefix" required value="lua/index.lua">
		    </div>
		    <div class="form-group col-md-12">
		      <label for="luarocks">LuaRocks Command:</label>
		      <input type="text" class="form-control" id="luarocks" placeholder="Enter LuaRocks command like install wsapi-xavante" name="luarocks">
		    </div>
			<div class="form-group col-md-12">
				<label class="head-label">STATUS</label>
			</div>
		    <div class="form-group col-md-2">
		    	<label class="">Lua: </label>
		    </div>
			...
			<div class="row row-margin">
			    ...
			    <div class="form-group col-md-3">
			    	<button type="button" class="btn btn-warning round-btn-med round-btn" onclick="act('luarocks', actionHandler)">EXECUTE LUAROCKS CMD</button>	
			    </div>
			    <div class="form-group col-md-1">
			    </div>
			</div>
			<div class="row row-margin">
			    <div class="form-group col-md-3">
			    	<button type="button" class="btn btn-success round-btn-med round-btn" onclick="act('luvitstart', actionHandler)">START</button>	
			    </div>
			    <div class="form-group col-md-1">
			    </div>
			    <div class="form-group col-md-3">
			    ...
			    <div class="form-group col-md-3">
					<button type="button" class="btn btn-warning round-btn-med round-btn" onclick="act('start', actionHandler)">EXECUTE LUA CMD</button>	
					 <!-- <button type="button" class="btn btn-warning round-btn-med round-btn" onclick="restartAct()">RESTART</button>	  -->
					<!-- <button type="button" class="btn btn-warning round-btn-med round-btn" onclick="act('luarocksinstall', actionHandler)">EXECUTE LuaRocks COMMAND</button>	 -->
			    </div>
			    <!--<div class="form-group col-md-3">-->
			    <!--	<button type="button" class="btn btn-info round-btn-med round-btn" onclick="act('luarocksstart')">LuaRocks START</button>	-->
			    <!--</div>-->
			    <div class="form-group col-md-1">
			    </div>
			</div>
		    ...
		    <div class="form-group col-md-4">
		      <label for="host">LuaDist Linux Bundle:</label>		      
		    </div>
		    <div class="form-group col-md-4">
				<input type="file" class="form-control" id="manual_file" placeholder="Upload LuaDist bundle for linux manually" name="manual_file" >
			</div>
			<div class="form-group col-md-2">
			...
			<div class="form-group col-md-12">
				<p>You can download LuaDist  from <a href="http://luadist.org/" target=”_blank”>here</a>, and upload it using the uploader above, or goto the <a href="../../fm" target=”_blank”>File Manager</a>, login using your superadmin credentials, and upload it to {{PRESTIGE_HOME}}/python/api, and then press the install button above.</p>
			</div>
