    ...
    for(var k in data){
        if(data.hasOwnProperty(k) && data[k].items && data[k].items.length > 0){
            for (var i = 0; i < data[k].items.length; i++) {
                if(!data[k].items[i].action.startsWith(prefix)) data[k].items[i].action =  prefix + data[k].items[i].action;
                ...
                if(data[k].items[i].items && data[k].items[i].items.length > 0){
                    ...
                    for (var j = 0; j < data[k].items[i].items.length; j++) {
                        if(!data[k].items[i].items[j].action.startsWith(prefix)) data[k].items[i].items[j].action = prefix + data[k].items[i].items[j].action;
